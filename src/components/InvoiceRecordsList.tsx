...
export default function InvoiceRecordsList({ records, loading, error, onEdit, onDeleted, editingId }: Props) {
  ...
  const handleDelete = async (record: InvoiceApiRecord) => {
    const confirmed = window.confirm(
      `Delete invoice ${record.invoiceNumber || "this invoice"} for ${record.customer?.name || "this customer"}? This cannot be undone — it will be permanently removed for everyone.`
    );
    if (!confirmed) return;
